...
public class Path extends Shape {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -4090103840529032686L;
	...
	public void curveTo(float x, float y, float cx1, float cy1, float cx2, float cy2, int segments) {
		...
		Curve curve = new Curve(new Vector2(cx,cy),new Vector2(cx1,cy1),new Vector2(cx2,cy2),new Vector2(x,y));
		float step = 1.0f / segments;
		
		for (int i=1;i<segments+1;i++) {
			float t = i * step;
			Vector2 p = curve.pointAt(t);
			if (hole != null) {
